...
import com.serte.testbackgrounddatafetch.threads.WorkThread11
...
class Fragment1 : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        view.findViewById<TextView>(R.id.txtPosition).text = "1"
        ...
        WorkHelper(context!!).execute(WorkThread1(), Events.EVENT_TAG_MESSAGE_USER_UPDATED,
                Callback<String> { result ->
                    edtText.setText(result.value)
                })

        WorkHelper(context!!).execute(WorkThread11(), Events.EVENT_TAG_MESSAGE_USER_UPDATED,
                Callback<String> { result ->
                    edtText.setText(result.value)
                })
